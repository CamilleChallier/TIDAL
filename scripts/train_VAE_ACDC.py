"""
Stage 1 training pipeline for the DVFVAE spatial β-VAE (ACDC cardiac dataset).
Cardiac analog of train_VAE.py: hard-codes ACDC loaders, folds, VOL_SIZE, and
the MambaMorph/VoxelMorph checkpoint in place of their liver counterparts.
...
from mopred.models import Voxelmorph, SpatialTransformer

from mopred.data.splits       import make_folds_acdc as make_train_val_test_folds
from mopred.data.data_loaders.acdc_4d import ACDC_4D_Dataset, CANONICAL_SHAPE, LIVER_SHAPE, get_cardiac_phase
...
# =============================================================================
# Phase helpers — cardiac analog of get_phi / get_cycle_phase (navigator_4d.py)
# =============================================================================
# ACDC gives the exact cardiac phase (get_cardiac_phase, ∈ [0,1], ED=0/1, ES=0.5)
# directly from Info.cfg, so both liver debug aliases are derived from it:
#   _get_phi          ∈ [0, 0.5] — distance from the ED reference (mirrors liver's phi)
#   _get_cycle_phase  ∈ [0, 1]   — full-cycle phase (identical to get_cardiac_phase)
...
_get_cycle_phase = get_cardiac_phase
...
def train_vae(
    ...
    # for split, seqs in zip(("train", "valid"), folds):
    #     build_dvf_cache(
    #         data_dir=cfg.data_dir, sequence_list=seqs, vm=vm,
    #         cache_dir=cache_dir,   device=device,
    #         nb_inputs=cfg.nb_inputs, nb_pred=cfg.tp,
    #     )

    # ---- data loaders ------------------------------------------------------
    ds_kwargs = _acdc_dataset_kwargs(cfg)
    train_base = ACDC_4D_Dataset(cfg.data_dir, nb_inputs=cfg.nb_inputs, nb_pred=cfg.tp, sequence_list=folds[0], **ds_kwargs)
    valid_base = ACDC_4D_Dataset(cfg.data_dir, nb_inputs=cfg.nb_inputs, nb_pred=cfg.tp, sequence_list=folds[1], valid=True, **ds_kwargs)

    # train_set = CachedDVF_Dataset(train_base, cache_dir)
    # valid_set = CachedDVF_Dataset(valid_base, cache_dir)
    train_set = train_base
    valid_set = valid_base
...
def compute_latent_stats(
    ...
    train_CLDM_ACDC can load it without retraining.

    Usage
    -----
    python -m 4D_MoPred_liver.scripts.train_VAE_ACDC --config configs/VAE/dvfvae_acdc.yaml \
    ...
    train_set = ACDC_4D_Dataset(
        ...
        **ds_kwargs)
    # train_set = CachedDVF_Dataset(train_set, cache_dir)
    loader = DataLoader(
        train_set, batch_size=cfg.batch_size,
        shuffle=False, num_workers=cfg.num_workers,
    )
...
def test_vae(
    ...
    phi_values   = []   # [n_samples, tp] — distance from ED ∈ [0, 0.5]
    cycle_phases = []   # [n_samples, tp] — full cycle, exact ∈ [0, 1]
    ...
    with torch.no_grad():
        for idx, (ref_volume_raw, input_volume_list, current_volume_list_raw, vol_file) in enumerate(
            ...
            if isinstance(vae, (IR_CVAE, IR_CVAE_new, IR_CVAE_nodec, IR_CVAE_compress, CVAE_old)):
                # Posterior reconstruction via dedicated method
                dvf_recon_list = vae.reconstruct_dvf_list(
                    ref_volume, dvf_gt_list, input_volume_list, device,
                )
            else:
                # VQVAE / MIA_VAE / MonaiVQVAE: forward(dvf) → {"recon": ...}
...
def _parse_cli() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Unified VAE trainer — ACDC")
...
if __name__ == "__main__":
    ...
    cfg.fold_nb_training = args.fold_nb_training   # CLI wins over YAML
