"""
DPM-Solver-v3 sampler with empirical model statistics for improved scheduling.
Adapted from the DPM-Solver-v3 repository (https://github.com/thu-ml/DPM-Solver-v3).
"""
import torch
...
class DPM_Solver_v3:
    def __init__(
        ...
        if statistics_dir is not None:
            l = np.load(os.path.join(statistics_dir, "l.npz"))["l"].astype(np.float32)
            sb = np.load(os.path.join(statistics_dir, "sb.npz"))
            s, b = sb["s"].astype(np.float32), sb["b"].astype(np.float32)
        else:
            assert degenerated, "statistics_dir is required when degenerated=False"
            _N = 1000
            l = np.ones(_N + 1, dtype=np.float32)
            s = np.zeros(_N + 1, dtype=np.float32)
            b = np.zeros(_N + 1, dtype=np.float32)
        if degenerated:
            l = np.ones_like(l)
            s = np.zeros_like(s)
            b = np.zeros_like(b)
    ...
    def multistep_predictor_update(self, x_lst, eps_lst, time_lst, index_lst, t, i_t, order=1, pseudo=False):
        ...
        time_0n = torch.stack(index_list(time_lst, indexes))
