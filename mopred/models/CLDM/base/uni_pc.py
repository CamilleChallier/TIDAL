...
Adapted from the UniPC repository (https://github.com/wl-zhao/UniPC).
"""
import torch
    ...
    def multistep_uni_pc_vary_update(self, x, model_prev_list, t_prev_list, t, order, use_corrector=True):
        ...
        if self.predict_x0:
            ...
            if len(D1s) > 0:
                ...
                for k in range(K - 1):
                    x_t = x_t - alpha_t * h_phi_ks[k + 1] * torch.einsum('bk...,k->b...', D1s, A_p[k])
            # now corrector
            if use_corrector:
                ...
                k = 0
                for k in range(K - 1):
                    x_t = x_t - alpha_t * h_phi_ks[k + 1] * torch.einsum('bk...,k->b...', D1s, A_c[k][:-1])
            ...
            if len(D1s) > 0:
                # compute the residuals for predictor
                for k in range(K - 1):
                    x_t = x_t - sigma_t * h_phi_ks[k + 1] * torch.einsum('bk...,k->b...', D1s, A_p[k])
            # now corrector
            if use_corrector:
                model_t = self.model_fn(x_t, t)
                D1_t = (model_t - model_prev_0)
                ...
                k = 0
                for k in range(K - 1):
                    x_t = x_t - sigma_t * h_phi_ks[k + 1] * torch.einsum('bk...,k->b...', D1s, A_c[k][:-1])
